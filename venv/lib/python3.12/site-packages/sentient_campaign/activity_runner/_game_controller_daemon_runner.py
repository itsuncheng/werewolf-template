...
class GameControllerDaemonRunner:
    ...
    def __init__(self,docker_client, campaign_name, activity_id, health_probe_port: int, comm_server_port: int = 8008):
        ...
        self.container = None
        self.game_controller_image = "public.ecr.aws/u2d4o0f0/sentient_foundation/sentient_campaign_werewolf_controller:0.3.6"
    ...
    def get_image(self):
        ...
        try:
            logger.debug(f"Pulling game controller image: {
    ...
    def copy_to_container(self, container, src, dest, dest_file_name):
        try:
            ...
            logger.error(f"Error copying files to container {
                         container.id}: {str(e)}")
            return False
        ...
        try:
            ...
            if not self._check_container_status(container):
                raise ContainerError(
                    container, 125, "Container failed to start")
    ...
    def initialize_game_controller(self, container_name, all_players: List[str]):
        ...
        try:
            ...
            logger.info(f"Initializing game controller , server url {matrix_server_url}")
            ...
            logger.info(f"Initializing game controller with command: {
                        " ".join(command_list)}")
            ...
            if exec_result.exit_code == 0 or exec_result.exit_code == None:
                logger.info(f"Initialization output: {
                            exec_result.output.decode('utf-8')}")
            else:
                logger.error(f"command failed to execute {
    ...
    def _wrap_redirection_to_container_exec_command(self, command: Union[str, list], posix_log_path: PurePosixPath, append=False):
    ...
    def start_game_controller(self, all_players: List[str],player_roles_mapping:Dict[str,str]=None):
        if not self.container:
            logger.error("No container found")
            raise ContainerError(self.container, 125, "Container not found")
        try:
            ...
            redirected_command = self._wrap_redirection_to_container_exec_command(command_list, PurePosixPath(controller_log_path))
            logger.info(f"Starting game controller with command: {" ".join(redirected_command)}")
    ...
    def check_if_game_is_running(self):
        ...
        try:
            ...
            if game_status in ["initializing", "running"]:
                ...
                return True
            elif game_status in ["ended", "terminated"]:
