...
class WerewolfCampaignActivityRunner(CampaignActivityRunner):
    ...
    def _copy_game_transcripts_and_logs(
        ...
        return transcrip_paths, log_paths
    ...
    def _print_werewolf_started_message(self):
          ...
          for line in werewolf.split("\n"):
                ...
                time.sleep(0.05)
          print("werewolf game started, you can now open the game in chrome browser using following")
    ...
    def _monitor_game(
        ...
        while not self.stop_flag.is_set():
            ...
            for player_name in all_players_infra:
                ...
                if (
                    not agent_container_runner.is_agent_running(player_name)
                    and player_name not in agent_failures
                ):
                    self.stop_flag.set()
                    ...
                    if not ignore_agent_failure:
                        return {
                            ...
                            "failure_game_stats": {
                                ...
                                "traceback": json.dumps(agent_error_tail),
                            },
                        }
                    else:
                        agent_failures[player_name] = {
                            "error_list": [json.dumps(agent_error_tail)],
                            "error_count": 1,
                        }
    ...
    def _monitor_game_and_save_result(
        ...
        if not path_to_final_transcript_and_log_dump is None:
            ...
            transcrip_path, log_paths = self._copy_game_transcripts_and_logs(
                all_players_agent_infra,
                path_to_final_transcript_and_log_dump,
    ...
    def log_game_result(self, final_game_stat):
        ...
        elif final_game_stat["game_status"] == "terminated":
            logger.error(
